...
// 如果当前环境是开发环境的话，那么就引入nprogress样式文件
...
router.beforeEach((to, from, next) => {
  ...
  */
    // 提交到vuex中改变state状态中的值
   store.commit('SET_BROWSERHEADERTITLE', {
     browserHeaderTitle: browserHeaderTitle // 这个是额外的参数
   })
  // 点击登录时，拿到了token并存入了cookie,保证页面刷新时,始终可以拿到token
  if (getToken('Token')) {
    // 因为前面已经获取到登录的token了，如果还是点击去登录页面的话，那么直接返回首页
    if(to.path === '/login') {
      // 重定向到首页
      next({ path: '/' })  
      NProgress.done() 
    ...
    // 判断要去的那个路由地址是否在whiteList数组中，如果存在就执行
    if (whiteList.indexOf(to.path) !== -1) {
      ...
      // next({path:"/xxxx"})，表示路由拦截成功，重定向到了"/xxxx"路由，会再次调用router.beforeEach()
      next('/login')
      NProgress.done()
    }
  }
...
/*
  vue全局的导航钩子函数
  router.afterEach()添加一个导航钩子，在每次导航后执行
  给每一个页面设置title
*/
router.afterEach(() => {
